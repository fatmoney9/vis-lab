...
| ID | 规则 | 实现 | 状态 |
|---|---|---|---|
| LEGEND-11 | **纵向单列排布（LEGEND-01 的 `column`）的溢出 = 容器内纵向滑动**：项数超出可用高度时图例区自身滚动，**不分页、不截断、不换列**，绘图区尺寸不受影响。<br>三主题一致——纵列是「图例在侧旁」这种布局的固有形态，溢出方式不随品牌分化，故**不进 `behavior.json`**。<br>⚠️ **纵向居中必须写成 `justify-content: safe center`，不能是裸 `center`**：同一个元素既要「不溢出时整列与绘图区中线对齐」（LEGEND-10）又是滚动容器，而**居中的溢出是往两头对半溢的**——上半截落到滚动原点之上，`scrollTop` 不能为负，那部分**永远滚不到**（36 项时实测首项在容器可视顶之上 362px，前八九项等于凭空消失，且滚到底也找不回来）。`safe` 的语义正是「一旦该对齐会导致溢出就退回 `start`」：不溢出照常居中，溢出时自动从顶部起排、整列可完整滚完。**这不是样式偏好而是可达性缺陷**——滚动条的存在会让人以为内容都在，实际首几项不可见也不可达。<br>**横排换行排布（`row`）的溢出同样是容器内纵向滑动**，但**只在饼环的上下结构下落地**（上限取值见 [pie.md](pie.md) PIE-09）：横排下**不需要** `safe`——多行的竖直分布归 `align-content`，其默认 `normal`(≈`stretch`) 本就从顶部起排，不存在纵列那种「居中把上半截推到滚动原点之上」的陷阱。<br>与 LEGEND-07 的关系：那条管的是横排换行后**按主题分化**的处置（THS 移动端滑动 / Web 分页器、iFinD 分页器、Ainvest 仅换行），本条给的是饼环上下结构这一处**不分主题**的兜底封顶；LEGEND-07 落地时按主题覆盖本条即可，两者不冲突 | `charts/styles.css` → `.dv-legend--column { overflow-y: auto; justify-content: safe center }`、`.dv-chart--legend-bottom .dv-legend { overflow-y: auto }` + 两种结构各自的 `.dv-chart__legend` 封顶 | ✅ |
| LEGEND-13 | **纵向单列排布（`column`）的标签超宽 → 省略号截断**；横排（`row`）**不截**。<br>**为什么只管纵列**：横排放不下会自动换行（LEGEND-01），长名称撑不爆宽度；纵列一项一行、不换行，一个长名称会一路把图例撑宽，而图例宽参与「图 + 间距 + 图例」整组居中的布局计算（[pie.md](pie.md) PIE-09），撑宽即整组偏移。<br>**上限 = `--size-legend-label-max`（三主题 120px）**。取固定 token 而非容器派生：图例宽既然参与整组居中，容器派生会让它随容器抖动、间距观感不稳。<br>**纯 CSS 实现**：图例标签是 HTML `<span>` 不是 SVG `<text>`，`text-overflow: ellipsis` 原生可用——不必测量、不必二分（对比 SVG 侧的 [pie.md](pie.md) PIE-16 要自己量着截）。<br>**完整名称由原生 `title` 兜底**：图例本就接受鼠标（要点击显隐），够得着；`title` 恒挂不判断是否真截了——判断要测量，而「没截时多一个与可见文字相同的 title」没有任何代价 | `charts/styles.css` → `.dv-legend--column .dv-legend-label`；`core/legend.js`（`title` 属性）；`tokens/*.json` → `size-legend-label-max` | ✅ |
