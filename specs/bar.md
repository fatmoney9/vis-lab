...
| ID | 规则 | 实现 | 状态 |
|---|---|---|---|
| BAR-01 | 柱渲染：**`null` 跳过**（断口，不画）· **`0` → `size-zero-bar-placeholder`（1px）** 贴基线细线 · 正值从 0 基线**向上**、负值**向下** · 圆角仅在**远离基线的一端**（正值圆顶、负值圆底），并按最终柱宽分档：柱宽 ≥ `size-bar-radius-full-min-width`（8px）取 `radius-bar-top`（THS=2px）；8px > 柱宽 ≥ `size-bar-radius-reduced-min-width`（4px）取 `radius-bar-top-reduced`（THS=1px）；柱宽 < 4px 降为 0。iFinD/Ainvest 的两档圆角 token 均为 0，所有宽度均保持直角 · 填充 = 系列色（`currentColor`） | `core/mark.js` → `renderBars()`；`.dv-bar`（styles.css） | ✅ |

## 排布

| ID | 规则 | 实现 | 状态 |
|---|---|---|---|
| BAR-02 | **分组排布**（`stack:none` 多柱系列，图表专属计算，留 L2）：x 走 `slot` 模式（band = 整格 step、**组间距最小 0**，同 BAR-03；组间距 = `step − 组内容` 为残量，数据少→容器封顶组间距变大、数据多→容器缩小组间距=0）。柱在 `container = min(step, size-bar-group-container-max)` 内、整组回 step 居中。柱宽上限 `size-bar-max`、柱间距上限 `size-bar-group-inner-gap-max`；**放不下时柱与间距按同一比例等比缩小**（恒保持 `size-bar-max : size-bar-group-inner-gap-max`，如 32:2；间距只在柱顶到 `size-bar-max` 时才是满值 2px，柱多则同比变窄）。**容器内左右留白**（`size-bar-group-gap-ratio`）：三主题 `2:1`——内容块(柱+柱间距) : 两侧留白总和 = 2:1，内容块只占 container 的 2/3、进一步压小柱与间距（例：2 柱各 32 + 间距 2 = 66 内容块 → 两侧留白 33 → container 99）；`0` 退化为不留侧白、内容块铺满 container。**整组宽受 `size-bar-group-container-max` 上限**：iFinD/Ainvest `100px`（band 宽于它时组不铺满），**THS `none`=不设上限**（`size-bar-max` 仅 16px、组自然窄，无需封顶；代码将 none/非正值读作 `Infinity`）。**隐藏系列按可见重排**：槽位按当前可见柱数重算，剩余柱整组重新居中（宽度仍受上述双上限约束）。系列色由固定 `--dv-series-i` 决定、与可见性无关，故颜色不重排（COLOR-04 仍成立） | `charts/charts/cartesian/index.js`（可见过滤 + 读 token）+ `layout.js`（groupedBars(…, containerMax)） | ✅ |
...
| BAR-05 | **堆叠**（`stack:normal`）：每类目**单列**（band 不分组，几何同基础单柱 `singleBar`——container 内留侧白 + `size-bar-max` 封顶、窄 band 等比收缩，见 BAR-03；所有系列段共用这一 `{offset,width}` 居中）；各系列段从上一段**累计基线**长起，**正值向上累计、负值向下累计**（分开）；**段间直角**，只有**整根堆叠的最外端**才圆角（沿用 BAR-01「远离基线端 + 按最终柱宽分档」：每类目正向**最上段**顶圆角、负向**最下段**底圆角；`stackBars` 逐类目算 `caps` 布尔喂 `renderBars(capped)`）；**0 不占位、null 跳过**（1px 占位是基础柱专属）；值域 = 堆叠总高（`niceSplit(min负累计, max正累计)`）。**隐藏系列按可见重算**（段闭合、轴 refit、caps 随可见段重定，与非堆叠的稳定轴不同） | `core/mark.js` 的 `base`/`capped` 参数 + `charts/charts/cartesian/layout.js` → `stackBars()` | ✅ |
| BAR-06 | **归一化堆叠**（`stack:percent`）：每类目缩放到 **100%**（占比 = `v / 类目正值和`，**假设正值**）；Y 轴固定 **0–100%**、百分比格式；**所有分段均为直角，不保留普通堆叠最外端圆角**（THS 同样为 0px）；其余排布同 BAR-05 | `charts/charts/cartesian/layout.js` `stackBars()` + `index.js`（`pctFormat` + `rounded:false`） | ✅ |
...
柱最大宽 `size-bar-max` · 单柱容器上限 `size-bar-container-max` + 单柱留白比 `size-bar-gap-ratio`（柱:两侧留白，三主题 `2:1`；`0`=不留侧白） · 圆角 `radius-bar-top` / `radius-bar-top-reduced` + 圆角宽度阈值 `size-bar-radius-full-min-width` / `size-bar-radius-reduced-min-width` ·
