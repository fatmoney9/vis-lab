...
| RADAR-06 | 多边形支持色彩填充：**默认态填充 10% 不透明度**（用于突出主体数据）；**选中整组时填充本色、非选中态压到 24%**。描边恒为系列本色、不随填充变化 | `opacity-radar-area` / `opacity-radar-area-dim`；`charts/styles.css` | ✅ |
