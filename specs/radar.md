...
| RADAR-13 | 虚线系列显示 | 待实现 | ⏳ |
| RADAR-14 | **可调节态的维度名称沿外围圆弧排布，不水平直排**。每段标签弧占相邻轴夹角的 76%，三轴时封顶 60°，以保留轴间断口；下半圆的路径反向以保持文字正向，并由 L1 `measureInk()` 按每条文字的**真实墨迹 ascent** 外移，使五个标签的墨迹内缘都从 `R + 4px` 开始。不得按整行高补偿——行高大于实际墨迹，会让下方标签显得更远。弧长就是 `truncateBatch` 的宽度上限，显示几何与测量口径一致。常规雷达继续使用 RADAR-07 的八向横排标签 | `core/measure.js` → `measureInk()`；`geometry.js` → `labelArc()`；`index.js` → `<textPath>` | ✅ |
