...
> 可调节态的文字环绕标签已实现（RADAR-14）；评级雷达、虚线与加载态本期未实现，见「待办」。
...
**下沉点 ②：绕圆标签几何**（`labelAnchor` / `labelArc`）
**判断条件**：仓库里出现**第二个**需要「把文字摆在圆周上、按所在方位决定对齐方式或生成可读弧线」的图型。
- 会触发：**仪表盘**——刻度标签沿弧排布，要回答的问题和雷达维度标签完全相同（*我在圆的哪一侧 → 用什么对齐方式、弧线是否需要反向*）。
- **饼环不算已有的第二方**：`pie/geometry.js` 的 `labelAnchor(a0, a1, R, innerR)` 吃的是一个扇区的两个角、返回 `{x, y, maxWidth}`，外侧标签只分左右两侧，没有八向逻辑。
- 命中后：将两项纯几何一起移到 `charts/core/`，两族 import 同一份。**反面做法是在新图型里照抄一份**——那正是 `SankeyChart` 当年三笔 L1 欠账的由来。
...
**三个下沉点互相独立**，命中 ② 不代表 ① 也该动。故 `labelAnchor` 与 `labelArc` 都只收角度、半径等纯几何参数，不收任何雷达专属结构，命中时是移动纯函数而非重写。
...
| RADAR-07 | 轴标签展示维度名称，**从 12 点方向起顺时针排列**，沿径向轴向外延展 **4px** 后放置；整体不超出图内区域，超宽走 `truncateBatch` 截断而非丢弃。<br>`axisValue`（默认 `false`）开启后在名称下方加一行数值。<br>⚠️ **有两条早期限制本页不采纳**：「仅 1~2 组数据时可展示数值」与「展示数值时雷达图不可交互，仅做数据展示」。两条均已被后续设计推翻——三组数据的轴标签同样带数值，且照常具备 hover 与扇形高亮。故常规雷达的数值开关与数据组数、交互能力无绑定。**可调节态是明确例外**：它以 slider 气泡与 `aria-valuenow` 呈现当前值，不显示第二行轴标签数值，也忽略遗留的 `axisValue:true` | `geometry.js` → `labelAnchor()`；`core/label.js` → `truncateBatch()`；`core/measure.js` | ✅ |
...
| RADAR-13 | 虚线系列显示 | 待实现 | ⏳ |
| RADAR-14 | **可调节态的维度名称沿外围圆弧排布，不水平直排**。每段标签弧占相邻轴夹角的 76%，三轴时封顶 60°，以保留轴间断口；下半圆的路径反向以保持文字正向，并由 L1 `measureInk()` 按每条文字的**真实墨迹 ascent** 外移，使五个标签的墨迹内缘都从 `R + 4px` 开始。不得按整行高补偿——行高大于实际墨迹，会让下方标签显得更远。弧长就是 `truncateBatch` 的宽度上限，显示几何与测量口径一致。<br>⚠️ **上限取「这个标签自己那条弧」的长度，不是统一按 `R + gap` 估一个数**：下半圆的弧按字形 ascent 外移，半径大一圈、弧就长一截（实测 R80 / gap4 / 60° / ascent12 时相差 **12.6px**），统一预算会把下半圆的标签白白截短。<br>⚠️ **字形墨迹必须量未截断的原文**：拿截断后的串再量会得到另一个 ascent，于是预算与渲染又用上了两套几何——正是本条要消掉的病。实现上截断与渲染共用同一个 `arcFor(i)`，不可能再漂。常规雷达继续使用 RADAR-07 的八向横排标签 | `core/measure.js` → `measureInk()`；`geometry.js` → `labelArc()`；`index.js` → `<textPath>` | ✅ |
...
| RADAR-18 | **可调节雷达图（Adjustable Radar Chart）**是现有雷达的输入能力，不是另一套图族：`editable: true` 时在每个数据点叠加径向手柄，拖动或方向键可修改值；Home / End 直达量程两端。<br>**命名不用“交互式雷达图”**：基础形态已经有 hover、扇形高亮与点击钉住，“交互式”无法区分是否可改数据。API 也不占 `variant`：`basic / rating` 表达网格视觉变体，`editable` 与 `gridShape / shape` 可正交组合；`shape: 'straight' | 'curve'` **两种闭合形态都支持拖动**，曲线档只替换数据轮廓的插值方式，手柄仍锚同一组真实数据点。曲线档虽继续遵守 RADAR-05、不显示常规系列圆点，但在**曲线与手柄内缘的接点**叠加一个 **4×4px** 编辑锚点，颜色取 `color-text-tertiary`。<br>可调节态以轴上 slider 为唯一主交互，**不渲染 RADAR-10 的扇形 hover 热区**，因此鼠标经过雷达区域不会出现灰色高亮块或维度 hover 气泡；拖动 / 键盘调值时仍显示当前维度数值气泡。只支持**单系列**，多系列直接抛错——同轴多组手柄会重叠，无法辨认编辑对象。指针值取「圆心→指针向量在目标径向轴上的正交投影」，而不是到圆心的斜距；只沿轴方向的位移改变值，最终夹在统一 `[0,max]` 量程。<br>⚠️ **可调节态的量程一经建立即冻结，不随编辑结果重算**：`editable` 会原地改数据，而重建（拖容器 / 切旋钮）本会按当前数据重求上界——两者相乘的后果是用户把各点拖低后一次 resize 就把标尺缩小，已设好的值跳到外圈、`aria-valuemax` 一起变，输入控件的标尺在用户手底下漂。故首次建立后复用同一把标尺。**基础形态不冻结**，那里数据只读，重算才跟得上图例显隐带来的值域变化。给了 `max` 时两条路本就一致，此约束只对省略 `max` 的自动档有意义。**数据焦点落在手柄朝圆心一侧的圆周边缘，不在圆心**：36px 手柄的圆心沿轴向外偏 18px，反算值时扣回同一偏移；手柄与内部双箭头整体旋转到和所在径向轴一致。内部两枚实心三角形均为**底边 12px、高 5px**，相邻底边的净间距为 **2px**，整体关于手柄圆心对称。手柄是数据图元，**轴标签仍按 RADAR-07 的 4px 间距定位，不为手柄额外预留空间**。hover、按下与键盘焦点均不改变手柄颜色或透明度。`editStep` 可选吸附；不给时拖动连续、键盘以一个网格环为步长。组件复制输入数组，不反向修改调用方数据；每次变化通过 `onChange({ series, dimension, seriesIndex, dimensionIndex, value, data })` 上抛，实例的 `getData()` 可读取当前副本。手柄使用 `role="slider"` 与 `aria-value*` | `geometry.js` → `radarValueAt()` / `snapRadarValue()`；`index.js`；`size-radar-handle-*` / `color-radar-handle-*` | ✅ |
...
- `playground/radar-preview.html`——**雷达专用对照面**，四形态 × 三主题十二张图同屏铺开，改一版 token 或几何能一眼看到全部影响。它**import 共享示例源**，故不像桑基独立面那样有「改示例要改两处」的漂移代价（见 [WORKFLOW](../WORKFLOW.md) 第七节）。
...
RadarChart(host, {
 ...
 axisValue = false,       // 常规雷达：轴标签是否带数值；editable 时忽略（RADAR-07/14/18）
