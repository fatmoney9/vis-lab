...
> 并**返回实际画出的标签盒**供调用方做避让（2026-09-17 加，当前消费者是 [callout.md](callout.md) CALLOUT-07
> 的障碍物集）——盒子由本模块给而不是调用方自己量，因为宽度是按 `classOf` 的完整 class 串测的，
> 调用方复制一份类名口径就会出现第二个测量出口；未测量的批次（`needsMeasure` 为假，如饼环外侧档）返回空数组，
> 那条路径刻意省掉一次 layout flush，为拿盒子再量一趟等于把省下的还回去。
