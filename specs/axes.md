...
| AXIS-08 | **outside 标签列宽**：每次重绘按当前刻度**一次性渲染测量**（隐藏 SVG 真实类名量宽，含 tabular-nums 等 Canvas 表达不了的特性，无估算误差、不裁字），**精确贴合、不附加余量**；缩放轴联动时列宽随标签变化即时调整。inside 布局的数据让位、X 轴标签碰撞判定（AXIS-06）、**数据标签碰撞判定（LABEL-06②）**的宽度测量全部**同源**（渲染级测量是唯一测量源，Canvas 估算路径已移除）。<br>⚠️ **「不用 Canvas」只对「宽」成立**：同模块的 `measureInk`（2026-08-19 加，供 AXIS-01 的 inside 标签按墨迹边定位）**用 Canvas**——垂直墨迹只由 font-family / -size / -weight / -style 决定，这几项能原样交给 Canvas，而 SVG 的 `getBBox()` 对 text 返回的是 em 盒（12px 字量出 17px 高）、根本拿不到墨迹。字体仍从 `getComputedStyle` 读真实级联，不猜 | `core/measure.js` → `measureTexts(host, texts, className)` · `measureInk(host, texts, className)`（零 import、可被 node 加载）；`core/axis.js` → `measureYLabelWidth()` 固定轴标签类名调用之 | ✅ |
...
- [x] inside 布局下 0 轴标签是否省略——**已决定并已实现：不省略**。最底的 0 标签恒压在轴线之上、不越下沿。（原文此处记的 `y+3` 配 `hanging` / `y-4` 的构造已于 2026-08-19 换掉，见下一条）
