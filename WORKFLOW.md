...
`demos/registry.js` 登记「图表类型 → L2 组件」，`demos/chart-presentation.js` 统一处理 Ainvest
图表内部内容的英文展示。各面 import 同一份、各自决定怎么展示，
...
**最大档这头也有一条约束：假数据池里任何两条系列都不许重复轨迹**——`posSeries` 的相位若一路
按 `index × 0.9` 递增，第 8 条落到 6.3、与 2π 只差 0.017，与第 1 条几乎同轨（实测 Δmax 仅为量程的
0.72%），而 7 色板恰好也在第 8 条循环回第 1 色，于是**同色又同高**，看起来像图表把同一条画了两遍。
故第 7 条起插半档相位回到区间前段。加宽任何一个池子时都要过这条守卫（`tests/examples.test.mjs`
遍历全部带滑块的示例，逐对比较最大档，阈值为量程的 3%）。
...
这里的“两类”只描述**组件的视觉与几何规范**。示例业务内容的语言不进 token / behavior：
...
静态门禁的各项检查集中在 `hooks/check.sh`——提交前守卫与 CI 调的是同一个脚本，故本地通过基本等于 CI 通过。
